<template>
    <div v-if="this.app.user" class="items">
        ...
        <h3 class="float-left">
            Users&nbsp;
            <small>
                <router-link :to="{ name: 'users.create' }"><font-awesome-icon :icon="['fas', 'plus-circle']" /></router-link>
            </small>
        </h3>
                ...
                <tbody v-if="items">
                    <tr v-for="item in items" :key="item.id">
                        ...
                        <td class="text-center" v-if="app.user.is_admin">
                            <router-link class="btn btn-primary btn-sm" :to="{ name: 'users.edit', params: { id: item.id } }">
                    ...
                    <tr v-if="items.length == 0"><td :colspan="app.user.is_admin ? 4 : 3">No entries</td></tr>
                </tbody>
            </table>
